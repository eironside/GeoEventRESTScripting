...
# If the list is empty, this script will operate on all inputs where the urlParamName is found (all others will be ignored)


urlParamName = "since"  # input the URL parameter name that should be set to the current datetime
...
def getTimeString(offset_sec=None):
    '''
...
def updateInputClientParameter(inputComponent):
    '''
    Iterates through the input transport properties and updates the client parameters property
    '''
    ...
    for tprop in inputComponent[TRANSPORT][PROPERTIES]:
        if str(tprop[NAME]).lower() == CLIENT_PARAMETERS.lower():
            ...
            preParams, discard = str(clientParametersValue).split(urlParamName)
            discard = discard.split("&", 1)
            postParams = ""
            if len(discard) > 1:
                postParams = "&{}".format(discard[1])
            newUrlParamValue = getTimeString(time_offset_sec)
            print "\tUpdating {} '{}' parameter value {} to {}".format(CLIENT_PARAMETERS, urlParamName, discard[0], newUrlParamValue)
            clientParametersValue = "{}{}={}{}".format(preParams, urlParamName, newUrlParamValue, postParams)
...
def Main():
    '''
    Connects to the GeoEvent Server and iterates through the inputs
    If an input is in the list of target inputs (or the list is empty) then it attemtps to modify that input
    '''
    ...
    if sh.token is not None:
        ...
        for component in componentList:
            if len(geeTargetInputList) > 0 and not any(component[NAME] in s for s in geeTargetInputList):
                ...
                try:
                    print "Updated {} '{}' parameter on input named {}".format(CLIENT_PARAMETERS, urlParamName, component[NAME])
                except:
